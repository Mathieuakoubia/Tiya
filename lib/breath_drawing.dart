// Routine 22 — Breath Drawing — Firebase RTDB
import 'dart:async';
import 'dart:math';
...
class BreathDrawing extends StatefulWidget {
  final String sessionId;
  final bool isEmitter;
  final String partnerName;
  final VoidCallback? onComplete;
  const BreathDrawing({super.key, required this.sessionId, required this.isEmitter,
    required this.partnerName, this.onComplete});
  @override State<BreathDrawing> createState() => _BreathDrawingState();
}

class _BreathDrawingState extends State<BreathDrawing>
  ...
  void initState() {
    ...
    if (widget.isEmitter) _ctrl.addListener(() => setState(() => _breathPhase = _ctrl.value));
  ...
  void _startExercise() {
    if (widget.isEmitter) {
      _ctrl.repeat();
      _sendTimer = Timer.periodic(const Duration(milliseconds: 100), (_) => _session.send('phase', _ctrl.value));
    }
  ...
  void _onFingerMove(Offset pos) {
    setState(() => _fingerPos = pos);
    if (!widget.isEmitter) {
      final size = MediaQuery.of(context).size;
      final amplitude = size.height * 0.12;
      final targetY = size.height / 2 + amplitude * sin(_breathPhase * 2 * pi + (pos.dx / size.width) * 2 * pi);
      setState(() => _score = (1.0 - (pos.dy - targetY).abs() / size.height * 2).clamp(0.0, 1.0));
    }
  }
  ...
  Widget _buildExercise() => GestureDetector(
    ...
    onPanUpdate: widget.isEmitter ? null : (d) => _onFingerMove(d.localPosition),
    onPanStart:  widget.isEmitter ? null : (d) => _onFingerMove(d.localPosition),
    behavior: HitTestBehavior.opaque,
    child: Stack(children: [
      const Positioned.fill(child: ColoredBox(color: _bg)),
      Positioned.fill(child: CustomPaint(
          painter: _WaveLinePainter(phase: _breathPhase, color: _lilas,
              showFinger: !widget.isEmitter, fingerPos: _fingerPos, score: _score))),
      Align(alignment: Alignment.topCenter, child: SafeArea(child: Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Text(widget.isEmitter
              ? 'Respirez naturellement.\n${widget.partnerName} trace votre souffle.'
              : 'Suivez la ligne avec votre doigt...',
  ...
  Widget _buildComplete() => Stack(key: const ValueKey('done'), fit: StackFit.expand, children: [
    ...
    SafeArea(child: Center(child: Padding(padding: const EdgeInsets.symmetric(horizontal: 36),
      child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        ...
        const Text("'Vos souffles se sont tracés ensemble.'", textAlign: TextAlign.center,
...
class _WaveLinePainter extends CustomPainter {
  final double phase, score;
  final Color color;
  final bool showFinger;
  final Offset? fingerPos;
  const _WaveLinePainter({required this.phase, required this.color,
    required this.showFinger, this.fingerPos, required this.score});
  @override
  void paint(Canvas canvas, Size size) {
    final amplitude = size.height * 0.12;
    final centerY = size.height / 2;
    final p = Paint()..color = color.withValues(alpha: 0.50)..strokeWidth = 2.5..style = PaintingStyle.stroke..strokeCap = StrokeCap.round;
    final path = Path();
    for (int i = 0; i <= 250; i++) {
      final x = (i / 250) * size.width;
      final y = centerY + amplitude * sin(phase * 2 * pi + (i / 250) * 2 * pi);
      if (i == 0) { path.moveTo(x, y); } else { path.lineTo(x, y); }
    }
    canvas.drawPath(path, p);
    if (showFinger && fingerPos != null) {
      final fc = Color.lerp(Colors.red, Colors.green, score)!;
      canvas.drawCircle(fingerPos!, 14, Paint()..color = fc.withValues(alpha: 0.60));
      canvas.drawCircle(fingerPos!, 14, Paint()..color = fc..style = PaintingStyle.stroke..strokeWidth = 2);
    }
  }
  @override bool shouldRepaint(_WaveLinePainter old) => old.phase != phase || old.fingerPos != fingerPos;
}
